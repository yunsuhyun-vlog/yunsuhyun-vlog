...
| **FPGA & Embedded Systems**<br>*(FPGA 및 임베디드 제어)* | **[FPGA(Basys 3) 기반 자율주행 RC카 제어 시스템](https://github.com/yunsuhyun-vlog/rc_car)** | - L298N BJT 스위칭 특성을 분석하여 PWM 주파수를 1kHz로 튜닝, 데드존(Deadzone) 최소화.<br>- UART 파싱 시 발생하는 하드웨어 타이밍 에러(Negative Slack)를 시프트 연산 및 16진수 비트 슬라이싱으로 최적화.<br>- 엔코더 노이즈 제거용 디바운스(Debounce) 필터 및 500ms 통신 단절 시 자동 정지하는 워치독(Watchdog) 하드웨어 안전 로직 구현. | `SystemVerilog`<br>`Basys 3 (FPGA)`<br>`PWM / UART`<br>`Encoder` |
