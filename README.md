...
| **Embedded Systems & Firmware**<br>*(임베디드 및 모터제어)* | **[FPGA & STM32 기반 RC카 모션 제어 시스템](https://github.com/yunsuhyun-vlog/rc_car)** | - L298N BJT 스위칭 특성을 분석하여 PWM 주파수를 1kHz로 튜닝, 데드존(Deadzone) 최소화.<br>- UART 파싱 시 발생하는 하드웨어 타이밍 에러(Negative Slack)를 시프트 연산 및 16진수 비트 슬라이싱으로 최적화.<br>- STM32 Input Capture 인터럽트를 활용한 단일 채널(A상) 엔코더 방향 판별 로직 구현. | `STM32 (C)`<br>`SystemVerilog`<br>`PWM / UART`<br>`Encoder` |
